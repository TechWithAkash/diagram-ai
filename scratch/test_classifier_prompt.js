// scratch/test_classifier_prompt.js
...
- For "dc-circuit": V, R
- For "general-series-circuit": V, f, R, L, C
- For "general-parallel-circuit": V, f, R, L, C
...
async function test() {
  const testCases = [
    "series RLC circuit with R = 25 ohm, L = 40mH",
    "parallel RLC circuit with R=10 ohm, L=0.1H",
    "State and explain Superposition Theorem",
    "A series RLC circuit with R = 20, L = 0.5H, C = 50uF connected to a 230V 50Hz supply"
  ];

  for (const q of testCases) {
    console.log(`\nQuery: "${q}"`);
    try {
      const res = await runClassifier(q);
